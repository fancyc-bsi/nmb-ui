...
const NessusControl = () => {
  ...
  // Handle initial SSH key loading
  useEffect(() => {
    const loadInitialSettings = () => {
      try {
        const savedSettings = localStorage.getItem('nmb_settings');
        if (savedSettings) {
  ...
  const useDefaultSSHKey = () => {
    try {
      ...
      console.error('Error loading default SSH key:', error);
    }
  };

  // Modify the remote key field in your form
  const renderRemoteKeyField = () => (
    <TextField
      fullWidth
      margin="normal"
      label="SSH Key File"
      name="remoteKey"
      value={controlData.remoteKey}
